...
#define DEFAULT_ALPHA          0.005      /**< parameter to increase the confidence width in UCB */
...
#define DEFAULT_USENCONSS        TRUE
//zhs: 这么调种子是一个不合适的行为
/*--------------------*/
/* individual random seeds */
#define DEFAULT_SEED 0
#define MUTATIONSEED 0
#define CROSSOVERSEED 0
/*--------------------*/
...
{  /*lint --e{715}*/
   ...
   //zhs
   /*--------------------*/
   if( varprio->usenconss )
   {
      if( varprio->nconss[ind1] > varprio->nconss[ind2] )
         return -1;
      else if( varprio->nconss[ind2] > varprio->nconss[ind1] )
         return 1;
   }
   ...
   )
{
   ...
   *success = FALSE;
   ...
   varprio.usenconss = heurdata->usenconss;
   ...
   SCIP_CALL( SCIPallocBufferArray(scip, &nconss, nbinintvars) );
   /*--------------------*/
   ...
   for( b = 0; b < nbinintvars; ++b )
   {
      ...
      //zhs
      /*--------------------*/
      nconss[nunfixedvars] = vargraph->nvarconss[SCIPvarGetProbindex(var)];
      assert(nconss[nunfixedvars] <= SCIPvarGetNLocksDownType(var, SCIP_LOCKTYPE_MODEL)+SCIPvarGetNLocksUpType(var, SCIP_LOCKTYPE_MODEL));
      /*--------------------*/
   ...
   )
{
   ...
   *success = FALSE;
   ...
   SCIP_VGRAPH* vargraph;
   SCIP_CALL( SCIPvariableGraphCreate(scip, &vargraph, FALSE, 1.0, NULL) );
   SCIP_CALL( SCIPallocBufferArray(scip, &nconss, *nfixings) );
   /*--------------------*/
   ...
   for( i = 0; i < *nfixings; ++i )
   {
      ...
      //zhs
      /*--------------------*/
      nconss[i] = vargraph->nvarconss[SCIPvarGetProbindex(fixedvar)];
      assert(nconss[i] <= SCIPvarGetNLocksDownType(fixedvar, SCIP_LOCKTYPE_MODEL)+SCIPvarGetNLocksUpType(fixedvar, SCIP_LOCKTYPE_MODEL));
      /*--------------------*/
      perm[i] = i;
   ...
   )
{
   ...
   SCIP_CALL( SCIPaddBoolParam(scip, "heuristics/" HEUR_NAME "/usenconss",
         "nconss from fixed variables be used for variable prioritization",
         &heurdata->usenconss, TRUE, DEFAULT_USENCONSS, NULL, NULL) );

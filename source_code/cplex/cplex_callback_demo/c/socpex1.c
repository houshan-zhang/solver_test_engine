...
checkkkt (CPXCENVptr env, CPXLPptr lp, int const *cone, double tol)
{
   ...
   dispMsg (resc, "x      = [");
   for (j = 0; j < cols; ++j)
      dispMsg (resc, " %+7.3f", x[j]);
   dispMsg (resc, " ]\n");
   dispMsg (resc, "dslack = [");
   for (j = 0; j < cols; ++j)
      dispMsg (resc, " %+7.3f", dslack[j]);
   dispMsg (resc, " ]\n");
   dispMsg (resc, "pi     = [");
   for (i = 0; i < rows; ++i)
      dispMsg (resc, " %+7.3f", pi[i]);
   dispMsg (resc, " ]\n");
   dispMsg (resc, "slack  = [");
   for (i = 0; i < rows; ++i)
      dispMsg (resc, " %+7.3f", slack[i]);
   dispMsg (resc, " ]\n");
   dispMsg (resc, "socppi = [");
   for (q = 0; q < qcons; ++q)
      dispMsg (resc, " %+7.3f", socppi[q]);
   dispMsg (resc, " ]\n");
   dispMsg (resc, "qslack = [");
   for (q = 0; q < qcons; ++q)
      dispMsg (resc, " %+7.3f", qslack[q]);
   dispMsg (resc, " ]\n");

   /* Test primal feasibility. */
   dispMsg (logc, "Testing primal feasibility ... ");
    ...
    */
   dispMsg (logc, "Testing dual feasibility ... ");
   for (i = 0; i < rows; ++i) {
      switch (sense[i]) {
      case 'L':
         if ( pi[i] < -tol ) {
            dispMsg (errc, "<= row %d has invalid dual multiplier %f.\n",
                     i, pi[i]);
            goto TERMINATE;
         }
         break;
   ...
   for (q = 0; q < qcons; ++q) {
      if ( socppi[q] < -tol ) {
         dispMsg (errc, "Quadratic constraint %d has invalid dual multiplier %f.\n",
                  q, socppi[q]);
         goto TERMINATE;
      }
   }
   for (j = 0; j < cols; ++j) {
      if ( cone[j] == NOT_IN_CONE && dslack[j] < -tol ) {
         dispMsg (errc, "dslack value for column %d is invalid: %f\n", j, dslack[j]);
         goto TERMINATE;
      }
   }
   dispMsg (logc, "ok.\n");
    ...
    */
   dispMsg (logc, "Testing complementary slackness ... ");
   for (i = 0; i < rows; ++i) {
      if ( fabs (slack[i]) > tol && fabs (pi[i]) > tol ) {
         dispMsg (errc, "Complementary slackness not satisfied for row %d (%f, %f)\n",
                  i, slack[i], pi[i]);
         goto TERMINATE;
      }
   }
   for (q = 0; q < qcons; ++q) {
      if ( fabs (qslack[q]) > tol && fabs (socppi[q]) > tol ) {
         dispMsg (errc, "Complementary slackness not satisfied for cone %d (%f, %f).\n",
                  q, qslack[q], socppi[q]);
         goto TERMINATE;
      }
   }
   for (j = 0; j < cols; ++j) {
      if ( cone[j] == NOT_IN_CONE ) {
         if ( fabs (x[j]) > tol && fabs (dslack[j]) > tol ) {
            dispMsg (errc, "Complementary slackness not satisfied for non-cone variable %f (%f, %f).\n",
                     j, x[j], dslack[j]);
            goto TERMINATE;
         }
      }
    ...
    */
   dispMsg (logc, "Testing stationarity ... ");
   /* Initialize sum = c. */
   ...
   for (q = 0; q < qcons; ++q) {
      ...
      norm = sqrt (norm);
      if ( fabs (norm) <= tol ) {
         dispMsg (warnc, "WARNING: Cannot test stationarity at non-differentiable point.\n");
         skip = 1;
         break;
      }
   ...
   for (j = 0; !skip && j < cols; ++j) {
      if ( fabs (sum[j]) > tol ) {
         dispMsg (errc, "Stationarity not satisfied at index %d: %f\n",
                  j, sum[j]);
         goto TERMINATE;
      }
   }
   ...
   dispMsg (logc, "KKT conditions are satisfied.\n");
...
main (void)
{
   ...
   if ( CPXgetstat (env, lp) != CPX_STAT_OPTIMAL ) {
      dispMsg (errc, "Cannot test KKT conditions on non-optimal solution.\n");
      goto TERMINATE;
   }

   /* Now test KKT conditions on the result. */
   if ( !checkkkt (env, lp, cone, TESTTOL) ) {
      dispMsg (logc, "Testing of KKT conditions failed.\n");
      dispMsg (errc, "Testing of KKT conditions failed.\n");
      goto TERMINATE;
   }

   dispMsg (resc, "KKT conditions are satisfied.\n");
   retval = 0;
TERMINATE:
   free (cone);
   if ( lp != NULL )

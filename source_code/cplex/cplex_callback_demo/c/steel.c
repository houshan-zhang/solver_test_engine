                     ...
                     double *flatinv, double *flatsell, int buildbycolumn)
{
   ...
   /* Now that the channels have the error message function, we can
      use CPXmsgstr (via dispMsg) for errors. */
   ...
   if ( status ) {
      dispMsg (cpxerror,"Optimization failed. Status %d.\n", status);
      goto TERMINATE;
   }

   status = CPXsolution (env, lp, &lpstat, &objval, NULL, NULL, NULL, NULL);
   if ( status ) {
      dispMsg (cpxerror, "Solution failed. Status %d.\n", status);
      goto TERMINATE;
   }
   ...
   *lpstat_p = lpstat;
   ...
   if ( status ) {
      dispMsg (cpxerror, "CPXgetx failed to get Make solution.  Status %d.\n",
               status);
      goto TERMINATE;
   }
   ...
   if ( status ) {
      dispMsg (cpxerror, "CPXgetx failed to get Inv solution.  Status %d.\n",
               status);
      goto TERMINATE;
   }
   ...
   status = CPXgetx (env, lp, flatsell, 2*numprod*tweeks,
                     3*numprod*tweeks-1);
   if ( status ) {
      dispMsg (cpxerror, "CPXgetx failed to get Sell solution.  Status %d.\n",
               status);
      goto TERMINATE;
   }
          ...
          CPXENVptr env, CPXLPptr lp)
{
   ...
   if ( status ) {
      dispMsg (cpxerror, "Could not change objective sense. Error %d\n",
               status);
      goto TERMINATE;
   }
   ...
   for (p = 0; p < numprod; p++) {
      ...
      if ( status ) {
         dispMsg (cpxerror, "%sfor product %d.  Error %d.\n",
